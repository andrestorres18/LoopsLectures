...
int Account::largestBalance(vector<Account> accounts)
{
    ...
    if(accounts.begin()==accounts.end()){
        ...
        for(int i = 0; i<accounts.size();i++){
            if((largestBalance==0) || (accounts[i].getBalance() > largestBalance)){
                largestAccount = i;
                largestBalance = accounts[i].getBalance();
            }
...
int Account::hasDuplicates(vector<Account> accounts)
{
		// YOUR CODE HERE
    for(int i =0; i<accounts.size();i++){
        for(int t = i+1; t<accounts.size(); t++){
            if((accounts[t].getType()==accounts[i].getType()) && (accounts[t].getAccNo()==accounts[i].getAccNo())){
                return true;
            }
        }
    }
...
{
		// YOUR CODE HERE
        // vector<Account>::iterator i = accounts.begin();
...
vector<Account> Account::combine(vector<Account> v1, vector<Account> v2)
{
    ...
    for(int i = 0; i < v2.size(); i++){
        for(int j = 0;j<result.size();j++){
            if((v2[i].getType()==result[j].getType())&&(v2[i].getAccNo() == result[j].getAccNo())){
